...
We will add a "Traceability & Ticket Identity" section to map your architectural modules to the new ticket registry.

Replace the **entire contents** of **`CONTEXT.md`** with this updated version:
...
## PRD Traceability & Ticket Identity

To avoid identifier collisions across these deep modules, all backend tickets from BE-10 onwards (including Customer Domains, Repair Lifecycle, Staff Queues, and Inventory) are centrally registered.
Always refer to `docs/backend-ticket-register.md` and `docs/backend-prd-traceability.md` for the single source of truth mapping domains to execution tickets.
```
