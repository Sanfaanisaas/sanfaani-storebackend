...
export const register = catchAsync(async (req, res) => {
  ...
  try {
    ...
    //  THIS IS THE FIX: We are now logging the actual error to the terminal
    console.error(" LOGIN SESSION CRASH:", error);
...
export const refresh = catchAsync(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        accessToken,
        user: rotated.user.toSafeObject(),
      },
    });
  } catch (error) {
...
export const revokeSession = catchAsync(async (req, res) => {
  ...
  if (cookieSessionId(req) === revoked.sessionId) {
    clearRefreshCookie(res);
  }
