...
const unavailable = () => new AppError("Evidence is unavailable", 404, [{ code: "evidence_unavailable", message: "Check the evidence reference and permissions" }]);
const forbidden = () => new AppError("You do not have permission to manage this evidence", 403, [{ code: "forbidden", message: "Your account cannot perform this action" }]);

const DOMAIN_CONFIG = Object.freeze({
  order: { Model: Order, ownerField: "userId", purposes: new Set(["order_receipt"]), customerPurposes: new Set(["order_receipt"]), staff: new Set([USER_ROLES.STORE_OPERATOR, USER_ROLES.FINANCE_OFFICER, USER_ROLES.OPS_MANAGER, USER_ROLES.SUPER_ADMIN]) },
...
const validatedSubject = (subjectType, subjectId, purpose) => {
  ...
  if (!config || !validId(subjectId)) throw new AppError("Invalid evidence subject", 400, [{ code: "evidence_subject_invalid", message: "A valid evidence domain and identifier are required" }]);
  if (!config.purposes.has(purpose)) throw new AppError("Invalid evidence category", 400, [{ code: "evidence_category_invalid", message: "That category is not valid for this domain" }]);
  return config;
};

const staffAllowedForRepair = (subject, purpose, user) => {
  if (user.role === USER_ROLES.OPS_MANAGER || user.role === USER_ROLES.SUPER_ADMIN) return true;
  if (user.role === USER_ROLES.TECHNICIAN) return subject.technician?.toString() === user.id && purpose !== "qc" && purpose !== "handover";
  if (user.role === USER_ROLES.QC_OFFICER) return purpose === "qc";
  if (user.role === USER_ROLES.STORE_OPERATOR) return ["repair_intake", "custody", "handover"].includes(purpose);
  return false;
};

const assertSubjectAccess = async ({ subjectType, subjectId, purpose, user }) => {
  const config = validatedSubject(subjectType, subjectId, purpose);
  const select = subjectType === "repair" ? `${config.ownerField} technician` : config.ownerField;
  ...
  if (user.role === USER_ROLES.CUSTOMER) {
    if (owner.toString() !== user.id || !config.customerPurposes.has(purpose)) throw unavailable();
    return { subject, owner };
  }
  const allowed = subjectType === "repair"
...
const findAuthorizedEvidence = async (evidenceId, user) => {
  if (!validId(evidenceId)) throw new AppError("Invalid evidence identifier", 400, [{ code: "evidence_identifier_invalid", message: "A valid evidence identifier is required" }]);
  const evidence = await Evidence.findById(evidenceId).select("+objectKey");
  if (!evidence) throw unavailable();
  await assertSubjectAccess({ subjectType: evidence.subjectType, subjectId: evidence.subject, purpose: evidence.purpose, user });
  return evidence;
};
...
const queueCleanupAfterFailedCompensation = async ({ taskType, evidenceId, key, actorId }) => {
  try {
    await queueEvidenceCleanup({ taskType, evidenceId, objectKey: key, actorId });
  } catch {
...
export const uploadEvidence = catchAsync(async (req, res) => {
  if (!req.file) throw new AppError("Evidence file is required", 400, [{ code: "evidence_file_required", message: "Attach one supported evidence file" }]);
  const { subjectType, subjectId, purpose } = req.body;
  const { owner } = await assertSubjectAccess({ subjectType, subjectId, purpose, user: req.user });
  ...
  await putEvidenceObject({ key, body: req.file.buffer, contentType: file.detectedMimeType, checksum: file.checksum });
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      await writeAuditLog(req.user.id, "EVIDENCE_UPLOADED", "Evidence", evidence._id, { subjectType, purpose, size: file.size }, session);
    });
    ...
    try {
      ...
      await queueCleanupAfterFailedCompensation({ taskType: "DELETE_ORPHAN", key, actorId: req.user.id });
    }
    throw error;
  } finally {
...
export const downloadEvidence = catchAsync(async (req, res) => {
  ...
  await writeAuditLog(req.user.id, "EVIDENCE_DOWNLOAD_AUTHORIZED", "Evidence", evidence._id, { subjectType: evidence.subjectType, purpose: evidence.purpose });
  res.json({ success: true, data: { url: access.url, expiresAt: access.expiresAt } });
});

export const deleteEvidence = catchAsync(async (req, res) => {
  const evidence = await findAuthorizedEvidence(req.params.id, req.user);
  if (evidence.retentionState === "DELETED") return res.json({ success: true, data: { id: evidence._id, deleted: true } });
  if (evidence.retentionState === "LEGAL_HOLD") throw new AppError("Evidence is retained and cannot be deleted", 409, [{ code: "evidence_legal_hold", message: "This evidence is under retention" }]);

  const session = await mongoose.startSession();
  try {
    await session.withTransaction(async () => {
      await Evidence.updateOne({ _id: evidence._id, retentionState: "ACTIVE" }, { $set: { retentionState: "DELETE_PENDING" } }, { session });
      await writeAuditLog(req.user.id, "EVIDENCE_DELETE_REQUESTED", "Evidence", evidence._id, { subjectType: evidence.subjectType, purpose: evidence.purpose }, session);
    });
  } finally {
    await session.endSession();
  }
  ...
  try {
    ...
    await queueCleanupAfterFailedCompensation({ taskType: "FINALIZE_DELETION", evidenceId: evidence._id, key: evidence.objectKey, actorId: req.user.id });
    return res.status(202).json({ success: true, data: { id: evidence._id, deleted: false, cleanupPending: true } });
  }

  const finalSession = await mongoose.startSession();
  try {
    await finalSession.withTransaction(async () => {
      await Evidence.updateOne({ _id: evidence._id, retentionState: "DELETE_PENDING" }, { $set: { retentionState: "DELETED", deletedAt: new Date() } }, { session: finalSession });
      await writeAuditLog(req.user.id, "EVIDENCE_DELETED", "Evidence", evidence._id, { subjectType: evidence.subjectType, purpose: evidence.purpose }, finalSession);
    });
  } catch (error) {
    await queueCleanupAfterFailedCompensation({ taskType: "FINALIZE_DELETION", evidenceId: evidence._id, key: evidence.objectKey, actorId: req.user.id });
    throw error;
