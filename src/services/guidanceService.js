...
const safe = (session) => ({
  ...
  requirementsSummary: session.requirementsSummary || {
    budget: session.budget, useCase: session.useCase, brands: session.brands, categories: session.categories, requiredFeatures: session.requiredFeatures,
  },
  ...
  recommendations: session.recommendations.map((item) => ({ variant: idText(item.variant), score: item.score, factors: item.factors, availability: item.availability })),
...
const activeEscalation = async (session, owner) => {
  ...
  return escalation ? {
    id: idText(escalation._id), status: escalation.status, question: escalation.question,
    response: escalation.response || null, advisorDisplayName: escalation.advisorDisplayName || null,
    createdAt: escalation.createdAt, updatedAt: escalation.updatedAt,
  } : null;
};
...
export const createGuidance = async ({ owner = null, budget = null, useCase = null, brands = [], categories = [], requiredFeatures = [] }) => {
  ...
  const requirementsSummary = { budget, useCase: typeof useCase === "string" ? useCase.slice(0, 120) : null, brands: brands.slice(0, 10), categories: categories.slice(0, 10), requiredFeatures: requiredFeatures.slice(0, 20) };
  const session = await GuidanceSession.create({
    owner, ...requirementsSummary, requirementsSummary, rulesVersion: RULES_VERSION, evaluatedAt: new Date(), staleAt: new Date(Date.now() + 1000 * 60 * 60 * 24 * 7),
    recommendations, status: recommendations.length ? "ACTIVE" : "NO_MATCH",
    resumeDigest: digest(rawToken), resumeExpiresAt: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30),
  });
...
export const listOwnedGuidance = async ({ owner, query }) => {
  ...
  const [sessions, total] = await Promise.all([GuidanceSession.find({ owner }).sort({ updatedAt: -1 }).skip(skip).limit(limit), GuidanceSession.countDocuments({ owner })]);
...
export const archiveGuidance = async ({ owner, id }) => {
  ...
  if (session.status !== "ARCHIVED") {
    session.status = "ARCHIVED"; session.archivedAt = new Date(); session.resumeRevokedAt = new Date();
    await session.save(); await writeAuditLog(owner, "GUIDANCE_ARCHIVED", "GuidanceSession", session._id, {});
  }
  return safe(session);
};
...
export const createGuidanceEscalation = async ({ owner, id, question }) => {
  ...
  if (!session) throw unavailable("Guidance session");
  if (typeof question !== "string" || question.trim().length < 3 || question.trim().length > 1000) throw new AppError("Guidance question is invalid", 400);
  try {
    ...
    return { id: idText(escalation._id), status: escalation.status, question: escalation.question, response: null, advisorDisplayName: null, createdAt: escalation.createdAt, updatedAt: escalation.updatedAt };
  } catch (error) {
...
export const recordGuidanceAdvisorResponse = async ({ advisor, id, response, displayName = null }) => {
  ...
  escalation.response = response.trim(); escalation.advisorDisplayName = displayName ? String(displayName).slice(0, 120) : null; escalation.status = "responded"; escalation.respondedAt = new Date(); escalation.active = false;
  await escalation.save();
  await createCustomerNotification({ recipient: escalation.customer, type: "guidance_advisor_response", title: "Guidance advisor response", safePreview: "An advisor has responded to your guidance question.", resourceType: "guidance", resourceId: escalation.guidanceSession, mandatory: false, eventKey: `guidance-response:${escalation._id}` });
