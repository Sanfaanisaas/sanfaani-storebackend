...
const requestDto = (item) => ({
  id: idText(item._id), organisationName: item.organisationName, organisationType: item.organisationType,
  contactName: item.contactName, contactEmail: item.contactEmail, contactPhone: item.contactPhone,
  requirements: item.requirements.map((line) => ({ id: idText(line._id), category: line.category, quantity: line.quantity, minimumSpecifications: line.minimumSpecifications, preferredCondition: line.preferredCondition, notes: line.notes || null })),
  budgetRange: { minimum: item.budgetMin, maximum: item.budgetMax },
  requiredBy: item.requiredBy || null, fulfilmentMode: item.fulfilmentMode, fulfilmentLocation: item.fulfilmentLocation || null,
  softwareAndLicensingNeeds: item.softwareAndLicensingNeeds || null, warrantyAndSupportNeeds: item.warrantyAndSupportNeeds || null,
  setupDeploymentNeeds: item.setupDeploymentNeeds || null, accessibilityNeeds: item.accessibilityNeeds || null, notes: item.notes || null,
  ...
  clarifications: item.clarifications.map((item) => ({ id: idText(item._id), question: item.question, response: item.response || null, requestedAt: item.requestedAt, respondedAt: item.respondedAt || null })),
...
const quoteDto = async (quote) => {
  ...
  return {
    id: idText(quote._id), requestId: idText(quote.request), version: quote.version,
    ...
    subtotal: quote.subtotal, tax: quote.tax, fees: quote.fees, fulfilmentCharge: quote.fulfilmentCharge,
    totalAmount: quote.totalAmount, currency: quote.currency, validUntil: quote.validUntil, termsVersion: quote.termsVersion,
    warrantySummary: quote.warrantySummary || null, supportSummary: quote.supportSummary || null, status: quote.status,
    superseded: Boolean(quote.superseded), conversionStatus: quote.conversionStatus, orderId: quote.order ? idText(quote.order) : null,
    document: documentMetadata(evidence), createdAt: quote.createdAt,
  };
};
...
const ensureRequirements = (requirements) => {
  ...
  return requirements.map((line) => {
    if (!line || typeof line.category !== "string" || !line.category.trim() || !Number.isSafeInteger(line.quantity) || line.quantity < 1 || line.quantity > 10000 || typeof line.minimumSpecifications !== "string" || !line.minimumSpecifications.trim()) throw invalid("Each procurement requirement needs a category, quantity, and minimum specifications");
    return { category: line.category.trim(), quantity: line.quantity, minimumSpecifications: line.minimumSpecifications.trim(), preferredCondition: ["new", "refurbished", "either"].includes(line.preferredCondition) ? line.preferredCondition : "either", notes: typeof line.notes === "string" ? line.notes.trim().slice(0, 1000) : null };
  });
};
export const createRequest = async ({ owner, input, idempotencyKey }) => {
  ...
  const normalized = {
    organisationName: input.organisationName.trim(), organisationType: input.organisationType, contactName: input.contactName.trim(), contactEmail: input.contactEmail.trim().toLowerCase(), contactPhone: input.contactPhone.trim(),
    requirements, budgetMin: Number.isSafeInteger(input.budgetMin) && input.budgetMin >= 0 ? input.budgetMin : null, budgetMax: Number.isSafeInteger(input.budgetMax) && input.budgetMax >= 0 ? input.budgetMax : null,
    requiredBy: input.requiredBy ? new Date(input.requiredBy) : null, fulfilmentMode: ["delivery", "pickup", "either"].includes(input.fulfilmentMode) ? input.fulfilmentMode : "either",
...
export const listRequests = async ({ owner, query }) => {
  ...
  const [items, total] = await Promise.all([ProcurementRequest.find({ customer: owner }).sort({ updatedAt: -1 }).skip(skip).limit(limit), ProcurementRequest.countDocuments({ customer: owner })]);
...
export const updateRequest = async ({ owner, id, input }) => {
  ...
  for (const field of ["organisationName", "contactName", "contactEmail", "contactPhone", "fulfilmentLocation", "softwareAndLicensingNeeds", "warrantyAndSupportNeeds", "setupDeploymentNeeds", "accessibilityNeeds", "notes"]) if (typeof input[field] === "string") request[field] = input[field].trim();
  ...
  request.version += 1; request.timeline.push({ status: request.status, at: new Date(), message: "Customer updated the request" });
  await request.save(); await writeAuditLog(owner, "CUSTOMER_PROCUREMENT_REQUEST_UPDATED", "ProcurementRequest", request._id, { version: request.version });
  return requestDto(request);
};
export const respondClarification = async ({ owner, id, clarificationId, response }) => {
  ...
  clarification.response = response.trim(); clarification.respondedAt = new Date(); request.status = "UNDER_REVIEW"; request.timeline.push({ status: request.status, at: new Date(), message: "Clarification received" });
  await request.save(); await writeAuditLog(owner, "CUSTOMER_PROCUREMENT_CLARIFICATION_RESPONDED", "ProcurementRequest", request._id, {});
  return requestDto(request);
};
...
export const decideQuotation = async ({ owner, id, decision, version, idempotencyKey }) => {
  ...
  const key = requireIdempotencyKey(idempotencyKey); const now = new Date();
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (existing.decision?.type) {
        if (existing.decision.idempotencyKey === key && existing.decision.type === (decision === "approve" ? "APPROVED" : "DECLINED")) { quote = existing; return; }
      ...
      if (existing.version !== version || existing.superseded || !existing.isActionable || existing.status !== "ISSUED" || existing.validUntil <= now) {
        if (existing.validUntil <= now && existing.status === "ISSUED") { existing.status = "EXPIRED"; existing.isActionable = false; await existing.save({ session }); }
      ...
      existing.status = decision === "approve" ? "APPROVED" : "DECLINED"; existing.isActionable = false; existing.decision = { type: decision === "approve" ? "APPROVED" : "DECLINED", at: now, idempotencyKey: key, version };
      await existing.save({ session }); quote = existing;
...
export const createStaffQuotation = async ({ actor, requestId, input }) => {
  ...
  const nextVersion = (await ProcurementQuotation.findOne({ request: request._id }).sort({ version: -1 }).lean())?.version + 1 || 1;
  ...
  const quote = await ProcurementQuotation.create({ request: request._id, customer: request.customer, version: nextVersion, lineItems: lines, subtotal: input.subtotal, tax: input.tax || 0, fees: input.fees || 0, fulfilmentCharge: input.fulfilmentCharge || 0, totalAmount: input.totalAmount, validUntil: input.validUntil, termsVersion: input.termsVersion, warrantySummary: input.warrantySummary || null, supportSummary: input.supportSummary || null, documentEvidence: input.documentEvidence || null });
  request.status = "AWAITING_DECISION"; request.timeline.push({ status: request.status, at: new Date(), message: "A quotation is ready for review." }); await request.save();
  await createCustomerNotification({ recipient: request.customer, type: "procurement_quotation_issued", title: "Procurement quotation ready", safePreview: "A quotation is ready for your review.", resourceType: "procurement_quotation", resourceId: quote._id, eventKey: `procurement-quote:${quote._id}` });
